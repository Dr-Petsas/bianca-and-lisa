...
def _verwaltung_start(sit: dict, text: str) -> dict | None:
    """Legacy-Faelle ohne Termindaten nehmen bewusst den neuen Ersatzweg.

    Produktion fragt zuerst Datum/Uhrzeit, bei "weiss nicht" Behandler und
    erst dann den Namen. Die nachfolgenden Tests pruefen gezielt weiterhin
    Namenssuche, Mehrdeutigkeit und Schreibwege.
    """
    aus = flow.zug(sit, text)
    s = gehirn.sammler(sit)
    if s["frage"] == "wann":
        aus = flow.zug(sit, "Den Zeitpunkt weiß ich nicht mehr.")
    if s["frage"] == "arzt":
        aus = flow.zug(sit, "Den Behandler weiß ich auch nicht mehr.")
    return aus
...
def test_absage_fluss_komplett():
    """Ohne bekannte Termindaten: nach Datum/Behandler-Unwissen Name suchen,
    Treffer MIT Anrede bestaetigen und erst bei Ja loeschen."""
    ...
    try:
        ...
        assert "sonst noch" in z3["text"].lower()
        assert "neuen termin" not in z3["text"].lower()

        z4 = flow.zug(sit, "Nein, danke.")
        assert z4 and z4.get("hangup")
    finally:
...
def test_absage_stallone_fallback_trotz_fremder_anrufernummer():
    ...
    agentFindPatientAppointments lieferte damals wegen der Nummer des
    ANRUFERS 404. Die fremde Kontakt-Nummer geht heute gar nicht mehr in die
    Patientensuche; Kartei-ID + masPatientLastDoctor bleiben der sichere
    Rückfall und müssen den echten Termin punktgenau finden."""
...
def test_absage_hinweis_im_einstiegssatz_filtert():
    ...
    try:
        sit = _sit()
        z1 = _verwaltung_start(sit, "Ich muss meinen Termin am Donnerstag absagen.")
        assert z1 and "nachname" in z1["text"].lower()
...
def test_absage_startet_neubuchung_nur_auf_ausdruecklichen_wunsch():
    """Nach dem Storno kein automatisches Buchen; ein klar neuer Wunsch geht."""
    echt_find = verwalten.kal.find_patient_appointments
    echt_cancel = verwalten.kal.cancel_by_id
    echt_anstossen = verwalten.hintergrund.anstossen
    verwalten.kal.find_patient_appointments = lambda t, c: dict(GEFUNDEN)
...
def test_auskunft_und_folgeabsage():
    ...
    try:
        ...
        assert z1 and "behandler" in z1["text"].lower()
        ...
        z4 = flow.zug(sit, "Ach, den können Sie absagen bitte.")
        assert z4 and "wirklich absagen" in z4["text"].lower()
        assert gehirn.sammler(sit)["modus"] == "absagen"
    finally:
        verwalten.kal.find_patient_appointments = echt_find
...
def test_verschieben_fluss_komplett():
    """Verschieben: unbekannte Bestandszeit -> Behandler/Name, dann Neu-Wunsch."""
    echt_find = verwalten.kal.find_patient_appointments
    ...
    try:
        ...
        z6 = flow.zug(sit, "Nein, danke.")
        assert z6 and z6.get("hangup")
        assert gehirn.sammler(sit)["modus"] == ""
    finally:
        verwalten.kal.find_patient_appointments = echt_find
...
def test_verschieben_alt_neu_trennung():
    ...
    try:
        sit = _sit()
        z1 = _verwaltung_start(sit, "Ich möchte meinen Termin am Donnerstag auf Freitag verschieben.")
        assert z1 and "nachname" in z1["text"].lower()
...
def test_verwaltung_kein_termin_gefunden():
    ...
    try:
        sit = _sit()
        z1 = _verwaltung_start(sit, "Ich möchte meinen Termin bei Doktor Petsas absagen.")
        assert z1 and "nachname" in z1["text"].lower()
        ...
        assert "sonst noch" in tl  # kein automatisches Abrutschen ins Buchen
...
def test_absage_varianten_erkannt():
    ...
    for satz in [
        ...
        sit = _sit()
        z = _verwaltung_start(sit, satz)
        assert gehirn.sammler(sit)["modus"] == "absagen", satz
        assert z and "nachname" in z["text"].lower(), f"{satz}: {z}"
    # Auch mit Zeitangabe im Einstiegssatz bleibt es bei der Nachnamen-Frage:
    sit = _sit()
    z = _verwaltung_start(sit, "Der Termin morgen fällt leider aus.")
    assert gehirn.sammler(sit)["modus"] == "absagen"
    assert z and "nachname" in z["text"].lower(), z
...
def test_absage_neustart_nach_notfound_mit_namenskorrektur():
    ...
    try:
        ...
        assert "nachname" in z4["text"].lower()
        s = gehirn.sammler(sit)
...
def test_absage_wiederholt_nach_abschluss_startet_neu():
    ...
    try:
        ...
        z4 = _verwaltung_start(sit, "Ich möchte meinen Termin absagen.")
...
def test_telefon_update_uebergibt_notiz_ohne_hartkodierte_stimme():
    ...
    s = gehirn.sammler(sit)
    s.update({
        "telefonAlt": "neu", "aktePhone": "01776004600",
    ...
    flow.kal.note_appointment = (
        lambda tenant, ctx, sit=None, *, note="":
...
def test_kein_angebot_nach_verschieben():
    ...
    try:
        ...
        # Verabschiedung danach: kein frisches Angebot und kein freies LLM,
        # sondern genau ein sauberer Gesprächsabschluss.
        ende = verwalten.zug(sit, "Das war's, danke schön.", set())
        assert ende and ende.get("hangup")
        assert "termin" not in ende["text"].lower()
    finally:
...
def test_besuchsgrund_ueberweiser_und_invisalign():
    ...
    assert kern == "Schiene/Schnarchen"
    assert vm and vm["name"] == "SLM Besprechung"
    # Pourianmehr 08.09.: Abholung einer fertigen Schiene → Eingliederung.
    satz = "Ich möchte meine Zahnschienen abholen und darüber möchte ich mit einem Arzt sprechen."
    assert besuchsgrund.ist_schiene_abholen(satz)
    kern, vm = besuchsgrund.deute(tenant, satz)
    assert kern == "Schiene abholen / Eingliederung"
    assert vm and vm["name"] == "SLM Eingliederung"
    kern, vm = besuchsgrund.deute(tenant, "Ich brauche eine Narval-Schiene")
    assert kern == "Schiene/Schnarchen"
    assert vm and vm["name"] == "SLM Besprechung"


def test_pourianmehr_schiene_abholen_startet_eingliederung():
    """Live 08.09.: nach Weiterleitungs-Frage muss Job die Eingliederung buchen."""
    ...
    try:
        ...
        s = gehirn.sammler(sit)
        assert s["modus"] == "buchen"
        assert s["grund"] == "Schiene abholen / Eingliederung"
        assert "Eingliederung" in (s.get("motivName") or "")
        assert z and z.get("text")
        assert not z.get("transfer") and not z.get("hangup")
        assert "Scan" not in (z.get("text") or "")
        assert "Resmed" not in (z.get("text") or "")
...
def test_anrufer_check_buchung_trennt_identitaet_terminempfaenger_und_sms():
    """Chef 10.09.2026: Name nur initial bestätigen, danach Pronomen. Die
    hinterlegte Nummer wird separat als SMS-Ziel vorgelesen und erst nach
    einem klaren Ja verwendet."""
    from bianca import agent as bianca_agent
...
def test_erkannt_michael_petsas_wird_nach_hallo_nur_noch_mit_sie_angesprochen():
    """Live 10.09.2026: Nach „Ah, Herr Petsas“ den Namen nicht drei- bis
    viermal wiederholen; auch gleicher Patienten-/Behandlername wird
    pronomenbasiert formuliert."""
    sit = _sit_mit_anrufer()
    sit["anrufer"].update({
        "vorname": "Michael", "nachname": "Petsas",
        "geschlecht": "male", "telefon": "+491776004600",
    })
    sit["anruferHalloGesagt"] = True  # „Ah, Herr Petsas“ lief bereits.
    sit["anruferKartei"] = {
        "letzterBesuch": "2026-03-01T09:00:00", "letzterGrund": "Kontrolle",
        "calendarId": "cal-petsas", "calendarName": "Dr. Michael Petsas",
        "doctorName": "Dr. Michael Petsas",
    }
    echt_anstossen = flow.hintergrund.anstossen
    flow.hintergrund.anstossen = lambda _sit: None
    try:
        z1 = flow.zug(sit, "Ich hätte gern einen Termin.")
        assert z1 and "Petsas" not in z1["text"]
        ...
        s.update({
            ...
            "slotIso": "2026-09-11T11:00:00+02:00",
        })
        rb = flow._readback(sit)["text"]
        assert "für Sie" in rb
        assert "für Herrn Petsas" not in rb
    finally:
        flow.hintergrund.anstossen = echt_anstossen
...
def test_anrufer_hallo_ohne_ziffern_und_gut_stoert_nicht():
    ...
    try:
        ...
        assert not s["nachname"]
        assert s["frage"] == "anrufer_check"
        assert z and "richtig erkannt" in z["text"]
...
def test_anrufer_hallo_frage_ist_eigener_zug_und_job_geht_danach_weiter():
    ...
    try:
        ...
        assert "Habe ich Sie richtig erkannt?" in z3["text"]
...
def test_anrufer_hallo_geht_als_vorab_nicht_seriell():
    ...
    try:
        ...
        bianca_agent.user_turn(
            sit, "Entschuldigung, bin ich beim Zahnarzt gelandet?",
            vorab=hits.append)
        assert not hits, "kein Hallo-Loop auf dem Talk-Pfad"
        # Latch weg: der Wächter allein muss den zweiten Hallo stoppen.
        sit["anruferHalloGesagt"] = False
        gehirn.sammler(sit)["frage"] = ""
        hits.clear()
        bianca_agent.user_turn(sit, "Morgen früh.", vorab=hits.append)
        assert not hits, "Wiederholungs-Wächter streicht den Vorab-Hallo"
    finally:
        flow.hintergrund.anstossen = echt_anstossen


def test_anrufer_hallo_nur_einmal_auch_ohne_checkfrage():
    """Live 08.09.2026: Vorab-Hallo, Maschine stellt nie anrufer_check
    (LLM-Talk) — trotzdem kein zweites „Ah, Herr …“."""
    sit = _sit_mit_anrufer()
    sit["anrufer"]["vorname"] = "Michael"
    sit["anrufer"]["nachname"] = "Petsas"
    sit["anrufer"]["geschlecht"] = "male"
    hallo = gehirn.anrufer_hallo_jetzt(sit, "Bin ich beim Zahnarzt gelandet?")
    assert hallo and "Herr Petsas" in hallo
    gehirn.anrufer_hallo_merken(sit)
    assert not gehirn.anrufer_hallo_jetzt(sit, "Ich hätte gern einen Termin.")
