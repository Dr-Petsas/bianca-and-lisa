"""Alle Eva-Thaler-Anrufe heute ausgeben."""
...
def main() -> None:
    ...
    if ROOT.is_dir():
        for d in ROOT.iterdir():
            ...
            if "thaler" not in blob and "eva" not in (m.get("patientName") or "").lower():
                continue
    ...
    for start, sid, m in rows:
        ...
        print("sammler", json.dumps(m.get("sammler") or {}, ensure_ascii=False)[:500])
        print("anrufer", json.dumps(m.get("anrufer") or {}, ensure_ascii=False)[:400])
        print("lastBook", json.dumps(m.get("lastBook") or {}, ensure_ascii=False)[:400])
        print("lastNote", json.dumps(m.get("lastNote") or {}, ensure_ascii=False)[:300])
        print("praxisNotiz", (m.get("praxisNotiz") or "")[:300])
        tools = m.get("tools") or []
        if tools:
            print("tools", json.dumps(tools, ensure_ascii=False)[:800])
