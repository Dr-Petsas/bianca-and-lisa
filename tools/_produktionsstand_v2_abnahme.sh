...
echo -n "Blessing-Ruhe-V2.9:      "; docker exec telefonki-bianca-1 python -c \
  'import json; from kern import sprech; t=json.load(open("/app/tenants/blessing.json")); k=("terminNotizNachBuchung","buchungAbschlussKompakt"); gruss="Hallo, Hautarztpraxis Doktor Blessing, Sie sprechen mit der digitalen Assistentin Bianca ...Wie kann ich helfen?"; assert all(t.get(x) is True for x in k); assert t.get("arztNotizFrageText"); assert "arztNotizAutomatisch" not in t; assert t.get("begruessungText") == gruss; assert sprech.ohne_krebs("Hautkrebs-Screening") == "Hautscreening"; print("OK")' \
  || exit 1
